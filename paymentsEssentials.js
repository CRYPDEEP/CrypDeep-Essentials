/**
 * payments.js
 * CrypDeep. Created by Suren Harutyunyan.
 *
 * This modern JavaScript file handles the checkout process.
 *
 ...
 * 2. It shows how to use the API to accept non-card payments,
...
(async () => {
  ...
  //This is only valid during the initial stage. Once you have obtained the API key,
  //you could save it in the config file to retrieve it locally.
  ...
  // Create a client. Change the country and currency codes.
  const stripe = Stripe("pk_test_Dl50EMtLjNMM4Fq4iCQ8lMxA"); //Example publishable key.
  const currency = 'eur';
  const country = 'ES';
  ...
  /**
   * Implement a Card Element that matches the look-and-feel of the app.
   *
  ...
  /**
   * Implement an IBAN Element that matches the look-and-feel of the app.
   *
 ...
 */
 ...
 const idealBank = elements.create('idealBank', {
  ...
  /**
   * Implement a Payment Request Button Element.
   *
  ...
  // Listen to changes to the user-selected cryptocurrency.
  form
  ...
  form.addEventListener('submit', async event => {
    ...
    var metadata = {
      status: 'created',
      total: form.querySelector('input[name=total]').value,
      address: form.querySelector('input[name=address]').value,
      crypto: form.querySelector('select[name=crypto] option:checked').value,
    };
    ...
    //Example of the Connect ID that should be passed in the metadata.
    metadata.connect = 'acct_1032D82eZvKYlo2C';
    ...
    if (payment === 'card') {
      // Create a source from the card information and the owner name.
      ...
      // Prepare all the source common data.
      const sourceData = {
        type: payment,
        amount: order.amount,
        currency: order.currency,
      ...
      // Create a source with the common data and extra information.
  ...
  const updateButtonLabel = (paymentMethod, bankName) => {
    ...
    if (paymentMethod !== 'card') {
      label = `Pay with ${name}`; // ${amount} with ${name}`;
    }
    if (paymentMethod === 'wechat') {
      label = `Generate QR code to pay with ${name}`; // ${amount} with ${name}`;
    }
    if (paymentMethod === 'sepa_debit' && bankName) {
      label = `Debit from ${bankName}`; // ${amount} from ${bankName}`;
    }
    submitButton.innerText = label;
  };

  const selectCrypto = crypto => {
    const selector = document.getElementById('crypto');
    selector.querySelector(`option[value=${crypto}]`).selected = 'selected';
    selector.className = `field ${crypto}`;
  };
  ...
  if (form.querySelector(`option[value="${countryParam}"]`)) {
    countryVar = countryParam;
  }
  selectCountry(countryVar);
})();
